...
def rgb_image(env: ManagerBasedRLEnv) -> torch.Tensor:
    """
    Returns the RGB image from the camera sensor.
    """
    image_output = env.scene["camera"].data.output["depth"]
    return image_output.flatten(start_dim=1)